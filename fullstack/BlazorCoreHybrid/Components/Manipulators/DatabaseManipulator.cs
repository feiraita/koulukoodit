...
namespace BlazorCoreHybrid.Manipulators {
    public class DatabaseManipulator {
        ...
        public static T SaveTodo<T>(T obj) where T : SaveableObject {
        ...
        /*
        public static bool DeleteTodoListItem(todoList item) {
            var table = typeof(todoListItem).Name
            var mongotable = database.GetCollection<todoListItem>(table);
            var filter = Builders<todoListItem>.Filter.Eq("_id", id);
            var results = mongotable.Find(filter);
        }
        */
    }
